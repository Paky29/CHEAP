...
#Stacking
#Models: Random Forest, Gradient Boosting, knn, ElasticNet
#Meta Model: SVR
...
def evaluate(individual, X_train, X_test, y_train, y_test):
    ...
    y_pred_en = elasticnet_regressor.predict(X_test_selected)
    y_pred_gb = gb_regressor.predict(X_test_selected)
    y_pred_knn = knn_regressor.predict(X_test_selected)

    X_meta = np.column_stack((y_pred_rf, y_pred_en, y_pred_gb, y_pred_knn))

    meta_regressor = SVR(kernel='linear')
...
def run(X,y):
    print('\n-Models: Random Forest, SVR, Gradient Boosting, knn\n-MetaModel: SVR')
    # split 70/30
    ...
    y_pred_en = elasticnet_regressor.predict(X_test)
    y_pred_gb = gb_regressor.predict(X_test)
    y_pred_knn = knn_regressor.predict(X_test)

    X_meta = np.column_stack((y_pred_gb, y_pred_rf, y_pred_en, y_pred_knn))
